...
import {
  ...
  ReportTableHeader,
  ReportTableTitle,
  ReportTableRow } from './NewReportStyle'

const NewReport = ({values, handleChange, setFieldValue, handleSubmit}) => {
  ...
  function addReportPiece(piece, amount){
    ...
    setPiecesTime([
      ...piecesTime,
      {
        hours: Math.floor(((piece.minutes * amount)% 60 + Math.floor((piece.seconds * amount)/60)/60)) + Math.floor((piece.minutes * amount)/ 60),
        minutes: ((piece.minutes * amount)% 60 + Math.floor((piece.seconds * amount)/60)%60),
  ...
  async function handleSearch(){
    await fetchData(searchPiece, sort, limit)
  }
...
export default withFormik({
  ...
  async handleSubmit(values, {props: {history}}){
    try {
      const { report: {pieces},  workers } = values
      ...
      ipc.send(SEND_CREATE_REPORT, {pieces: piecesData, workers})
